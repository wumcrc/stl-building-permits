...
```{r}
neighborhoods <- c("Forest Park South East", "Central West End", "DeBaliviere Place", "Skinker DeBaliviere", "West End", "Visitation Park", "Academy", "Fountain Park", "Lewis Place", "Vandeventer")

#create comments to use to filter and name outputs
comment(cwe_tiles) <- "cwe_tiles"
comment(fpse_tiles) <- "fpse_tiles"
comment(sdb_tiles) <- "sdb_tiles"
comment(dbp_tiles) <- "dbp_tiles"
comment(we_tiles) <- "we_tiles"
comment(vp_tiles) <- "vp_tiles"
comment(ac_tiles) <- "ac_tiles"
comment(fp_tiles) <- "fp_tiles"
comment(lp_tiles) <- "lp_tiles"
comment(vd_tiles) <- "vd_tiles"

for (i in list(cwe_tiles, fpse_tiles, sdb_tiles, dbp_tiles, we_tiles, vp_tiles, ac_tiles, fp_tiles, lp_tiles, vd_tiles)) {
  name <- comment(i)
  if(name %in% df$tiles){
  nbhd_pres <- filter(df, tiles == name)
  neighborhood <- unique(nbhd_pres$nbhd_name)
abbr <- unique(nbhd_pres$abbr)
num <- unique(nbhd_pres$Nbrhd)
...
summary <- block_list(
  ...
  fpar(ftext(paste(pct_chg_cnt, "change ", sep = " "), color24a),
       ftext(paste("compared to ", params$month, " ", params$pastyear, " (", bp_past, " total building permits)", sep = ""), reg24)),
  ...
  fpar(ftext(paste(pct_chg_cost, "change", sep = " "), color20a),
       ftext(paste(" compared to ", params$month, " ", params$pastyear, " (", cost_past, ")", sep = ""), reg20)),
  fpar(ftext(sprint_2, boldline)),
  fpar(ftext(paste(pct_chg_ytd, "change ", sep = " "), color24b),
       ftext(paste("YTD compared to this time in ", params$pastyear, " (", bptot_past, " total building permits)", sep = ""), reg24)),
  ...
  fpar(ftext(paste(pct_chg_cost_pres, "change", sep = " "), color20b),
       ftext(paste(" YTD compared to this time in ", params$pastyear, " (", totcost_past, ")", sep = ""), reg20))
)
...
if("Institutional" %in% nbhd_pres$new_use){col <- c(col, "Institutional" = "#984ea3")}
...
file.remove(file.path(mydir, dir(path = mydir, pattern = "bldgprmt_report")))
