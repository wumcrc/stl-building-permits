...
Once you have checked to see that the data is available for the current month, the next step is to change the `params` of the `data_cleaning.Rmd`. 
          ...
          `month6` should be set to 6 months prior to the first of the CURRENT month. An easy trick for this is just taking `enddate` and subtracting the month by 7 (Make sure you change the year as well if 6 months prior is not in the current year).
Once you have changed all of the params you can run the `data_creation.Rmd`.
