...
#ytd <- read_csv(here("data", params$year, params$month,  "ytd.csv"))
...
df%>%
  mutate(lrgcost = ifelse(grepl("1", big_cost), "> $50,000", "< $50,000")) -> df
```
...
ggsave(here("results", params$year, params$month, "allnbhd_plot.png"), width = 7, height = 4, units = "in", dpi = 500)
...
```{r, echo=FALSE}
knitr::include_graphics(here("results", params$year, params$month, "allnbhd_plot.png"))
```
...
fpse20 <- filter(df, Nbrhd == 39)
fpse19 <- filter(lastyear, Nbrhd == 39)
...
bp20 <- nrow(fpse20)
bp19 <- nrow(fpse19)
cost20 <- mean(fpse20$EstProjectCost)
cost20 <- currency(cost20, digits = 0L)
cost19 <- mean(fpse19$EstProjectCost)
...
sprint_7 <- sprintf("%s total crimes in %s", 
                    bptot20, params$year)
...
#print summary table
  knitr::kable(table,
               ...
               format="markdown", caption = "Forest Park Southeast")
}
```
...
#print summary table
  knitr::kable(table,
               col.names = c("Owner", "New Use", "Cost", "Address"),
               format="markdown", caption = "Forest Park Southeast")
}}
```
...
#print summary table
  knitr::kable(table,
               col.names = c("New Use", "Average Cost", "Permit Count"),
               format="markdown", caption = "Central West End")
  }
```
...
#print summary table
  knitr::kable(table,
               col.names = c("New Use", "Average Cost", "Permit Count"),
               format="markdown", caption = "DeBaliviere Place")
  }
```
...
sprint_7 <- sprintf("%s building permits in Skinker DeBaliviere in %s %s",
...
sprint_10 <- sprintf("%s building permits in West End in %s %s",
...
```{r, include=FALSE}
bp_vstprk <- nrow(df[df$Nbrhd == 49,])

sprint_9 <- sprintf("%s building permits in Visitation Park in %s %s",
                    bp_vstprk, params$month, params$year)
```
...
#save map as .jpeg
tmap_save(vstprk_map, file = here("results", params$year, params$month, "vstprk", "vstprk_use.jpeg"), dpi = 500)
}
```
...
#print summary table
  knitr::kable(table,
               col.names = c("New Use", "Average Cost", "Permit Count"),
               format="markdown", caption = "Visitation Park")
}
```
...
#print summary table
  knitr::kable(table,
               col.names = c("New Use", "Average Cost", "Permit Count"),
               format="markdown", caption = "Academy")
}
```
...
sprint_5 <- sprintf("%s building permits in Fountain Park in %s %s",
...
#print summary table
  knitr::kable(table,
               col.names = c("New Use", "Average Cost", "Permit Count"),
               format="markdown", caption = "Fountain Park")
}
```
...
knitr::include_graphics(here("results", params$year, params$month, "ftnprk", "ftnprk_cost.png"))
}
```
...
sprint_6 <- sprintf("%s building permits in Lewis Place in %s %s",
...
#print summary table
  knitr::kable(table,
               col.names = c("New Use", "Average Cost", "Permit Count"),
               format="markdown", caption = "Lewis Place")
}
```
...
sprint_8 <- sprintf("%s building permits in Vandeventer in %s %s",
...
#print summary table
  knitr::kable(table,
               col.names = c("New Use", "Average Cost", "Permit Count"),
               format="markdown", caption = "Vandeventer")
}
```
