...
# Compiles Year to Date Dataset for this year | Leave at January 1st of the current year
  ytd: "2020-01-01"
...
nbhd21 <- filter(df, Nbrhd == 39)
nbhd20 <- filter(lastmonth, Nbrhd == 39)
tot21 <- filter(ytd, Nbrhd ==39)
tot20 <- filter(lastytd, Nbrhd == 39)

#create cost and year to date variables for each year
bp21 <- nrow(nbhd21)
bp20 <- nrow(nbhd20)
cost21 <- mean(nbhd21$EstProjectCost)
cost21 <- currency(cost21, digits = 0L)
if(is.nan(cost21)){cost21[is.nan(cost21)] <- 0}
cost20 <- mean(nbhd20$EstProjectCost)
cost20 <- currency(cost20, digits = 0L)
if(is.nan(cost20)){cost20[is.nan(cost20)] <- 0}
bptot21 <- nrow(tot21)
bptot20 <- nrow(tot20)
totcost21 <- mean(tot21$EstProjectCost)
totcost21 <- currency(totcost21, digits = 0L)
if(is.nan(totcost21)){totcost21[is.nan(totcost21)] <- 0}
...
pct_chg_cost <- (cost21 - cost20)/cost20
...
pct_chg_ytd21 <- (bptot21 - bptot20)/bptot20
pct_chg_ytd21 <- percent(pct_chg_ytd21)
if(is.nan(pct_chg_ytd21)){pct_chg_ytd21[is.nan(pct_chg_ytd21)] <- 0}
if(is.infinite(pct_chg_ytd21)){pct_chg_ytd21[is.infinite(pct_chg_ytd21)] <- "Infinite"}
pct_chg_cost21 <- (totcost21 - totcost20)/totcost20
pct_chg_cost21 <- percent(pct_chg_cost21)
if(is.nan(pct_chg_cost21)){pct_chg_cost21[is.nan(pct_chg_cost21)] <- 0}
if(is.infinite(pct_chg_cost21)){pct_chg_cost21[is.infinite(pct_chg_cost21)] <- "Infinite"}

#create sprints for powerpoint output
sprint_1 <- sprintf("%s building permits in %s %s",
                    bp21, params$month, params$year)
...
color24b <- fp_text(color = ifelse(pct_chg_ytd21 == 0, '#00B0F0', ifelse(pct_chg_ytd21 > 0, '#00B050', 'red')), font.size = 24)
color20b <- fp_text(color = ifelse(pct_chg_cost21 == 0, "#00B0F0", ifelse(pct_chg_cost21 > 0, "#00B050", "red")), font.size = 20)
...
summary <- block_list(
  ...
  fpar(ftext(paste(pct_chg_cnt, "change ", sep = " "), color24a),
       ftext(paste("compared to ", params$month, " ", params$pastyear, " (", bp20, " total building permits)", sep = ""), reg24)),
  fpar(ftext(paste(cost21), bold),
  ...
  fpar(ftext(paste(pct_chg_cost, "change", sep = " "), color20a),
       ftext(paste(" compared to ", params$month, " ", params$pastyear, " (", cost20, ")", sep = ""), reg20)),
  fpar(ftext(sprint_2, boldline)),
  fpar(ftext(paste(pct_chg_ytd21, "change ", sep = " "), color24b),
       ftext(paste("YTD compared to this time in ", params$pastyear, " (", bptot20, " total building permits)", sep = ""), reg24)),
  fpar(ftext(paste(totcost21), bold),
       ftext(paste(": YTD average building permit cost in", params$year, sep = " "), reg24)),
  fpar(ftext(paste(pct_chg_cost21, "change", sep = " "), color20b),
       ftext(paste(" YTD compared to this time in ", params$pastyear, " (", totcost20, ")", sep = ""), reg20))
)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 38)
nbhd20 <- filter(lastmonth, Nbrhd == 38)
tot21 <- filter(ytd, Nbrhd ==38)
tot20 <- filter(lastytd, Nbrhd == 38)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 47)
nbhd20 <- filter(lastmonth, Nbrhd == 47)
tot21 <- filter(ytd, Nbrhd == 47)
tot20 <- filter(lastytd, Nbrhd == 47)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 46)
nbhd20 <- filter(lastmonth, Nbrhd == 46)
tot21 <- filter(ytd, Nbrhd == 46)
tot20 <- filter(lastytd, Nbrhd == 46)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 48)
nbhd20 <- filter(lastmonth, Nbrhd == 48)
tot21 <- filter(ytd, Nbrhd == 48)
tot20 <- filter(lastytd, Nbrhd == 48)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 49)
nbhd20 <- filter(lastmonth, Nbrhd == 49)
tot21 <- filter(ytd, Nbrhd == 49)
tot20 <- filter(lastytd, Nbrhd == 49)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 51)
nbhd20 <- filter(lastmonth, Nbrhd == 51)
tot21 <- filter(ytd, Nbrhd == 51)
tot20 <- filter(lastytd, Nbrhd == 51)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 53)
nbhd20 <- filter(lastmonth, Nbrhd == 53)
tot21 <- filter(ytd, Nbrhd == 53)
tot20 <- filter(lastytd, Nbrhd == 53)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 54)
nbhd20 <- filter(lastmonth, Nbrhd == 54)
tot21 <- filter(ytd, Nbrhd == 54)
tot20 <- filter(lastytd, Nbrhd == 54)
...
#filter out neighborhood
nbhd21 <- filter(df, Nbrhd == 58)
nbhd20 <- filter(lastmonth, Nbrhd == 58)
tot21 <- filter(ytd, Nbrhd == 58)
tot20 <- filter(lastytd, Nbrhd == 58)
