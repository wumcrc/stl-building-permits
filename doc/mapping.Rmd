...
neighborhoods[!(neighborhoods %in% sub$Nbrhd)] -> missing_category_days
is_empty(missing_category_days) -> test

as.data.frame(sub) -> sub

if(test == FALSE) {
  add_row(sub, Nbrhd = missing_category_days) -> sub
}
sub%>%
  replace(., is.na(.), 0) -> sub

nbhd_sf <- left_join(nhoods_sf, sub, by = c("neighborhood" = "Nbrhd")) %>% 
...
add_slide(layout = "Two Content")%>%
  ph_with(external_img(here::here("data", "neighborhood_pics", "stl_all copy.png")),
          location = ph_location_left())%>%
  ph_with(external_img(here::here("data", "neighborhood_pics", "nbhd_close.png"), height = 3.73, width = 4.19),
...
df%>%
  ...
  dplyr::select(nbhd_name, mean, count) -> table
...
month6%>%
  group_by(nbhd_name)%>%
  summarize(mean = mean(EstProjectCost), count = n())%>%
  dplyr::select(nbhd_name, mean, count) -> table1
...
#make summary tables
  ...
  set_header_labels(nbhd_name = "Neighborhood", 
    mean = "Average Cost", count = "# of Permits") -> flex
...
df%>%
  ...
  group_by(new_use)%>%
  summarize(mean = mean(EstProjectCost), count = n())%>%
  dplyr::select(new_use, mean, count) -> table
...
month6%>%
  ...
  group_by(new_use)%>%
  summarize(mean = mean(EstProjectCost), count = n())%>%
  dplyr::select(new_use, mean, count) -> table1
...
print(report, target = here::here("results", "presentations",  params$year, params$month, paste("monthly_report", params$month, params$year, ".pptx", sep = "")))
```
