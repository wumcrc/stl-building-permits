...
params:
  ...
  # Compiles Data for the most recent month of the previous year | This is the last day of month |      Must be the 30th or 31st (w/exception of February)
  pstmonth2: "2019-01-31"  
---
...
data%>%
  ...
  mutate(yr = year(as.Date(data$FirstDate,  format="%d/%m/%Y")))%>%
  mutate(month = month(as.Date(data$FirstDate, format="%d/%m/%Y")))%>%
  ...
  mutate(lrgcost = ifelse(grepl("L", big_cost), "> $50,000", "< $50,000"))%>% 
...
data_year <- filter(df, FirstDate >= as.Date(params$ytd) & FirstDate <= as.Date(params$enddate))
write_csv(data_year, here("data", params$year, params$month, "ytd.csv"))

#last year
lstyr <- filter(df, yr = params$pastyear)
write_csv(lstyr, here("data", params$year, "lastyear.csv"))


#last year this month
lstmonth <- filter(df, FirstDate >= as.Date(params$pstmonth) & FirstDate <= as.Date(params$pstmonth2))
write_csv(lstmonth, here("data", params$year, params$month, "lastmonth.csv"))

#last year to date
lastytd <- filter(df, FirstDate >= as.Date(params$pastdate) & FirstDate <= as.Date(params$pstmonth2))
write_csv(lastytd, here("data", params$year, params$month, "lastytd.csv"))
```
