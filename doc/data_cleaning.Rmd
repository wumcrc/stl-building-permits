...
data%>%
  ...
  dplyr::select(HANDLE, OwnerName, OrigAddress, AppDate, IssueDate,CompleteDate, CancelDate, EstProjectCost, AppDescription, NewUseGroup1, Nbrhd, FirstDate, big_cost, lil_cost, lrgcost, nbhd_name, yr, month, abbr) -> df
...
#current month
data_month <- filter(df, IssueDate >= as.Date(params$date) & IssueDate <= as.Date(params$enddate))
