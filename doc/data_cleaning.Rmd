...
data%>%
  ...
  mutate(new_use = ifelse(grepl("R2", NewUseGroup1), "Residential",
                                             ...
                                             ifelse(grepl("B", NewUseGroup1), "Commercial",
  ...
  mutate(tiles = ifelse(grepl("38", Nbrhd), "cwe_tiles",
                                             ifelse(grepl("39", Nbrhd), "fpse_tiles",
                                             ifelse(grepl("46", Nbrhd), "sdb_tiles",  
                                             ifelse(grepl("47", Nbrhd), "dbp_tiles",
                                             ifelse(grepl("48", Nbrhd), "we_tiles",
                                             ifelse(grepl("49", Nbrhd), "vp_tiles",       
                                             ifelse(grepl("51", Nbrhd), "ac_tiles",       
                                             ifelse(grepl("53", Nbrhd), "fp_tiles",       
                                             ifelse(grepl("54", Nbrhd), "lp_tiles",       
                                             ifelse(grepl("58", Nbrhd), "vd_tiles", NA)))))))))))%>%
  ...
  dplyr::select(AppType, CancelType, CityBlock, Parcel, OwnerCode, HANDLE, AddrNum,
                    ...
                    nbhd_name, yr, month, abbr, tiles) -> df
```
...
data_year <- filter(df, IssueDate >= as.Date(params$ytd) & IssueDate <= as.Date(params$enddate))
write_csv(data_year, here::here("data", params$year, params$month, "ytd.csv"))

#last year this month
lstmonth <- filter(df, IssueDate >= as.Date(params$pstmonth) & IssueDate <= as.Date(params$pstmonth2))
write_csv(lstmonth, here::here("data", params$year, params$month, "lastmonth.csv"))

#last year to date
lastytd <- filter(df, IssueDate >= as.Date(params$pastdate) & IssueDate <= as.Date(params$pstmonth2))
...
#Quarterly Data
data_qrt <- filter(df, IssueDate >= as.Date(params$qrtstart) & IssueDate <= as.Date(params$qrtend))
write_csv(data_qrt, here::here("data", params$year, "quarterly", params$quarter, "quarter-data.csv"))

#Last year quarterly data
data_qrt <- filter(df, IssueDate >= as.Date(params$qrtstart2) & IssueDate <= as.Date(params$qrtend2))
